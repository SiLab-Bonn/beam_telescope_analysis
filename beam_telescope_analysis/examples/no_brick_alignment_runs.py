...
output_folder = os.path.join(output_dir, 'no_brick')
if not os.path.exists(output_folder):
    os.makedirs(output_folder)
...
output_folder = os.path.join(output_dir, 'output_folder_no_brick')
if not os.path.exists(output_folder):
    os.makedirs(output_folder)
...
# prealigned_configuration = os.path.join(output_folder,'no_brick_telescope.yaml')
...
aligned_configuration = dut_alignment.align(
    ...
    select_duts=[[1, 2, 5, 6,9,10]],# [1, 2, 5, 6,],],
    select_telescope_duts=[1,9],
    select_fit_duts=[[1, 2, 5, 6,9,10]],# [1, 2, 5, 6,9,10],],
    select_hit_duts=[[1, 2, 5, 6,9,10]],# [1, 2, 5, 6,9,10],],
    max_iterations=[5],
    max_events= [100000],
...
aligned_configuration = os.path.join(output_folder,'no_brick_telescope.yaml')
...
track_analysis.fit_tracks(
    ...
    select_duts=[1, 2, 5, 6, 9, 10],
    select_fit_duts=[1,2,5,6,9,10],#[1,2,5,6], [1, 3, 5, 7]],
    select_hit_duts=[[2, 5, 6,9,10],[1,5,6,9,10],[1,2,6,9,10],[1,2,5,9,10],[1,2,5,6,10],[1,2,5,6,9]],#[1,2,5,6], [1, 3, 5, 7]],
    exclude_dut_hit=True,
    quality_distances=(500.0, 500.0),
    reject_quality_distances = (1000., 1000.),
    use_limits=True,
    plot=True)
...
#     select_duts=[1,5,11],
#     select_hit_duts=[[3, 5, 7,9,11],[1,3,7,9,11],[1,3,5,7,9]],
#     select_no_hit_duts=None,
#     select_quality_duts=[1, 3, 5, 7,9,11],
#     select_no_quality_duts=None,
#     condition='(track_chi2 < 20)')
#
...
result_analysis.calculate_efficiency(
    telescope_configuration=aligned_configuration,
    input_tracks_file=os.path.join(output_folder, 'Tracks_aligned.h5'),
    input_cluster_files= None, #[cluster_files[5]],
    output_efficiency_file=os.path.join(output_folder, 'Efficiency.h5'),
    select_duts=[1,5,10],
    resolutions=(50.,50.),
    extend_areas=None,
    plot_ranges= None, #[[(5000, 11000), (-3000, -9000)]],
    minimum_track_density=1,
    cut_distances=(750.,750.))
