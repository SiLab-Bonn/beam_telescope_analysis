...
cdef extern from "AnalysisFunctions.h":
    ...
    void histogram_1d(const unsigned int * & x, const unsigned int & rSize, const unsigned int & rNbinsX, uint32_t * & rResult) except +
    void histogram_2d(const unsigned int * & x, const unsigned int * & y, const unsigned int & rSize, const unsigned int & rNbinsX, const unsigned int & rNbinsY, uint32_t * & rResult) except +
    void histogram_3d(const unsigned int * & x, const unsigned int * & y, const unsigned int * & z, const unsigned int & rSize, const unsigned int & rNbinsX, const unsigned int & rNbinsY, const unsigned int & rNbinsZ, uint16_t * & rResult) except +
...
def hist_3d(cnp.ndarray[cnp.int32_t, ndim=1] x, cnp.ndarray[cnp.int32_t, ndim=1] y, cnp.ndarray[cnp.int32_t, ndim=1] z, const unsigned int & n_x, const unsigned int & n_y, const unsigned int & n_z, cnp.ndarray[cnp.uint16_t, ndim=1] array_result, throw_exception=True):
    histogram_3d(< const unsigned int*& > x.data, < const unsigned int*& > y.data, < const unsigned int*& > z.data, < const unsigned int&> x.shape[0], < const unsigned int&> n_x, < const unsigned int&> n_y, < const unsigned int&> n_z, < uint16_t*& > array_result.data)
